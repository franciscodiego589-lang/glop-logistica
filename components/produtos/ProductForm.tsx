...
export default function ProductForm({ productId, initial }: { productId: string | null; initial: Record<string, any> }) {
  ...
  const [f, setF] = useState<Record<string, any>>({
    name: str(initial.name), sku: str(initial.sku), code: str(initial.code), barcode: str(initial.barcode),
    gtin: str(initial.gtin), short_description: str(initial.short_description),
    product_type: initial.product_type ?? "finished_good", base_uom_code: initial.base_uom_code ?? "un",
    category_id: str(initial.category_id), brand_id: str(initial.brand_id), model: str(initial.model),
    segment: str(initial.segment), abc_class: initial.abc_class ?? "none", xyz_class: initial.xyz_class ?? "none",
    ncm: str(initial.ncm), cest: str(initial.cest), cfop_default: str(initial.cfop_default), tax_origin: str(initial.tax_origin),
    net_weight_g: str(initial.net_weight_g), gross_weight_g: str(initial.gross_weight_g),
    length_mm: str(initial.length_mm), width_mm: str(initial.width_mm), height_mm: str(initial.height_mm),
    cubage_m3: str(initial.cubage_m3), stack_max: str(initial.stack_max),
    cost_price: str(initial.cost_price), sale_price: str(initial.sale_price),
    min_stock: str(initial.min_stock), max_stock: str(initial.max_stock), reorder_point: str(initial.reorder_point),
    safety_stock: str(initial.safety_stock), lead_time_days: str(initial.lead_time_days),
    requires_lot: !!initial.requires_lot, requires_validity: !!initial.requires_validity,
    requires_serial: !!initial.requires_serial, is_perishable: !!initial.is_perishable, is_hazardous: !!initial.is_hazardous,
  ...
  async function onSave() {
    ...
    if (!tenantId && !productId) { setError("Empresa não resolvida."); return; }
    setSaving(true);
    const body: Record<string, any> = {
      ...
      is_perishable: f.is_perishable, is_hazardous: f.is_hazardous, active: f.active,
    };
    let err;
    if (productId) {
      ({ error: err } = await supabase.from("products").update(body).eq("id", productId));
    } else {
      ({ error: err } = await supabase.from("products").insert({ ...body, tenant_id: tenantId, company_id: COMPANY }));
    }
    if (!err) await supabase.rpc("compute_data_quality", { p_company: COMPANY }).then(() => {}, () => {});
    setSaving(false);
    if (err) { setError(err.message); return; }
    setOk(true);
    if (!productId) { router.push("/produtos"); router.refresh(); }
    else router.refresh();
  }
  ...
  return (
    <div className="card">
      <div className="flex gap-1 p-2 border-b overflow-x-auto" style={{ borderColor: "var(--border)" }}>
        {TABS.map((t) => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-3 py-1.5 rounded-lg text-sm whitespace-nowrap ${tab === t ? "bg-brand-600 text-white" : "hover:bg-black/5 dark:hover:bg-white/5"}`}>{t}</button>
        ))}
      </div>
      <div className="p-4">
        {tab === "Identificação" && (
          <Grid>
            ...
            <Field label="Ativo"><Select value={f.active ? "1" : "0"} onChange={(v) => set("active", v === "1")} options={[["1","Ativo"],["0","Bloqueado"]]} /></Field>
          </Grid>
        )}
        {tab === "Classificação" && (
          <Grid>
            <Field label="Tipo"><Select value={f.product_type} onChange={(v) => set("product_type", v)}
              options={[["finished_good","Acabado"],["raw_material","Matéria-prima"],["component","Componente"],["packaging","Embalagem"],["consumable","Consumível"],["kit","Kit"],["service","Serviço"],["other","Outro"]]} /></Field>
      ...
      <div className="p-4 border-t flex items-center gap-3" style={{ borderColor: "var(--border)" }}>
        ...
        {ok && <div className="text-sm text-green-500">Salvo ✓</div>}
        <div className="ml-auto flex gap-2">
          <Link href="/produtos" className="px-4 py-2 rounded-lg border text-sm" style={{ borderColor: "var(--border)" }}>Voltar</Link>
...
function Field({ label, span2, children }: { label: string; span2?: boolean; children: React.ReactNode }) {
  return <div className={span2 ? "md:col-span-2" : ""}><label className="text-xs font-semibold muted">{label}</label><div className="mt-1">{children}</div></div>;
}
function Input({ value, onChange, type = "text" }: { value: any; onChange: (v: string) => void; type?: string }) {
  return <input type={type} value={value} onChange={(e) => onChange(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm bg-transparent outline-none focus:border-brand-500" style={{ borderColor: "var(--border)" }} />;
}
function Select({ value, onChange, options }: { value: any; onChange: (v: string) => void; options: [string, string][] }) {
  return <select value={value} onChange={(e) => onChange(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm bg-transparent outline-none focus:border-brand-500" style={{ borderColor: "var(--border)" }}>{options.map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select>;
}
