...
export default async function IntegracoesPage() {
  ...
  const [dash, apis, connectors, webhooks, events, messages, flows, apiKeys] = await Promise.all([
    ...
    supabase.from("webhooks").select("*").eq("company_id", company).is("deleted_at", null).order("created_at", { ascending: false }).limit(100),
    ...
    supabase.from("api_keys").select("*").eq("company_id", company).is("deleted_at", null).order("created_at", { ascending: false }).limit(100),
  ]);
